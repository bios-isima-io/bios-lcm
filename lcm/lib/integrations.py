...
#
"""Module that manages biOS integrations"""

from lib.bios_container_common import retrieve_container_name, retrieve_current_version
from lib.common import execute_on_hosts, get_name_and_ip, run_remote, wait_for_server_up
...
def upgrade_integrations(config: dict, options: dict):
    """Upgrades integrations containes"""
    list_only = options.get("list_only")

    base_container_name = "bios-integrations"
    ...
    for tenant in target_tenants:
        ...
        for host in compute_nodes:
            ...
            command, image_name, image_version = build_docker_create_command(
                ...
                env_vars=[
                    "ALLOW_RESTART_ON_CONFIG_CHANGE",
                    "BIOS_ENDPOINT",
                    "APPLICATIONS",
                    "BIOS_TENANT",
                ],
            ...
            https_service_port = None
            http_service_port = None
            https_bindings = (
                container_properties.get("HostConfig", {}).get("PortBindings").get("8443/tcp", [])
            )
            for binding in https_bindings:
                https_service_port = binding.get("HostPort")
                break
            if not https_service_port:
                http_bindings = (
                    container_properties.get("HostConfig", {})
                    .get("PortBindings")
                    .get("8081/tcp", [])
                )
                for binding in http_bindings:
                    http_service_port = binding.get("HostPort")
                    break
            ...
            try:
                ...
                if renamed:
                    Log.error(
                        "Failed to create new bios container from image"
            ...
            try:
                ...
                if https_service_port:
                    service_url = f"https://{host['ip']}:{https_service_port}"
                elif http_service_port:
                    service_url = f"http://{host['ip']}:{http_service_port}"
                if service_url:
                    wait_for_server_up(None, service_url)
            except RuntimeError as error:
                if renamed:
                    Log.error(
